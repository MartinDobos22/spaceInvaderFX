...
public class Controller implements Initializable {
    ...
    private void startGame(){
        ...
        bulletTimer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                int n = Alien.aliens.size();
                int randomNumber = random.nextInt(n);
                newRandomBullets(randomNumber);
            }
        },1000,100);

    }

    private void newRandomBullets(int randomNumber){
    ...
    private void draw(){
        ...
        } else if(Alien.aliens.size() == Alien.killedAliens){
            gc.setFont(new Font(48));
            gc.fillText("GG YOU WIN", canvas.getWidth() / 2 - 125, canvas.getHeight() / 2);
        }
    }
    ...
    private void collision(){
        Alien.aliens.forEach(alien -> {
            ...
            if(
                    ...
                    playersBullet.getX() + playersBullet.getWidth() > alien.getY() &&
    ...
    private void handleInput(){
        ...
        if(input.contains("D")){
            if (player.getX() + player.getSpeedX() >= canvas.getWidth()){
                return;
            }
            player.setX(player.getX() + player.getSpeedX());
        }
        if(input.contains("SPACE") && player.isWeaponReady()){
            playersBullet.setX(player.getX() + player.getWidth() /2);
            playersBullet.setY(player.getY());
            player.setWeaponReady(false);
        }
    }
